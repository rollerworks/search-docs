...
your integration, or using RollerworksSearch as a SDK for (REST) API's.
...
lower bound and exclusive lower bound (everything lower then the value itself).
...
The ``SearchConditionBuilder`` has really powerful and developer friendly, but
        ...
        ->group()
            ...
            ->group(ValuesGroup::GROUP_LOGICAL_OR)
                ->field('first-name')
                    ...
                    ->add(new PatternMatch('(spider|pig)', PatternMatch::PATTERN_REGEX))
                    ->add(new PatternMatch('(doctor|who)', PatternMatch::PATTERN_REGEX, true)) // case-insensitive match
                ->end()
            ->end()
        ->end()
        ->getSearchCondition();
